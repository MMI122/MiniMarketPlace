...
public class ApiProductController {
    ...
    @PostMapping
    public ResponseEntity<Map<String, String>> addProductApi(@RequestBody ProductDto productDto, Authentication authentication) {
        if (authentication == null || !authentication.isAuthenticated()) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
        }
        productService.addProduct(productDto, authentication.getName());
    ...
    @PutMapping("/{id}")
    public ResponseEntity<Map<String, String>> updateProductApi(@PathVariable Long id, @RequestBody ProductDto productDto, Authentication authentication) {
        if (authentication == null || !authentication.isAuthenticated()) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
        }
    ...
    private Map<String, Object> mapToSafeJson(Product p) {
        ...
        map.put("sellerName", p.getSeller().getUsername());
        return map;
    }
}